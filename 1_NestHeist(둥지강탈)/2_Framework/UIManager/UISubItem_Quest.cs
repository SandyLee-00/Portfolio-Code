...
public class UISubItem_Quest_Presenter : IListener
{
    ...
    private CompletedAndRewardBody MakeCompletedAndRewardBody()
    {
        ...
        ServerUserGameInfoData _userData = DataManager.Instance.ServerData.User;
        _userData.Exp += exp;

        while (true)
        {
            if (_userData.Level == GameManager.Instance.UserManager.MaxLevel && exp != 0)
            {
                Logging.LogError("UserSystem::LevelUpCheck - PlayerLevelUp is null 최고레벨");
                break;
            }

            if (_userData.Exp < DataManager.Instance.BaseData.PlayerLevelUp[_userData.Level].SumOfExp)
            {
                break;
            }

            // 이전 레벨 저장
            if (GameManager.Instance.UserManager.TempPrevLevel == -1)
            {
                GameManager.Instance.UserManager.TempPrevLevel = _userData.Level;
            }

            _userData.Exp -= DataManager.Instance.BaseData.PlayerLevelUp[_userData.Level].SumOfExp;
            _userData.Level++;

            Logging.Log($"UserSystem::LevelUpCheck {_userData.Level}");
        }
        ...
        {
            UserId = DataManager.Instance.ServerData.User.UserId,
            Exp = DataManager.Instance.ServerData.User.Exp,
            Level = DataManager.Instance.ServerData.User.Level
        };
    ...
    public async void OnClickCompleteButtonPostCompletedAndReward()
    {
        ...
        await LoadingManager.Instance.RunWithLoading(async () =>
        {
            ...
            await DataManager.Instance.ServerData.PostCompletedAndReward(
       ...
       onLoaded: () =>
       {
           ...
           if (GameManager.Instance.UserManager.TempPrevLevel != -1)
           {
               ...
               popuplevelup.SetText("레벨 업 " + GameManager.Instance.UserManager.TempPrevLevel + " -> " + DataManager.Instance.ServerData.User.Level);
               GameManager.Instance.UserManager.TempPrevLevel = -1;
           }
