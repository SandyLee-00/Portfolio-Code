...
public class QuestSystem : IListener
{
    ...
    {
        Dictionary<string, QuestInfoData> questDataBase = DataManager.Instance.BaseData.Quest;
        ServerSubtaskDic = DataManager.Instance.ServerData.SubtaskList.ToDictionary(x => x.Id);
    ...
    {
        List<ServerCompletedQuestData> completedQuestList = DataManager.Instance.ServerData.CompletedQuestDataList;
    ...
    {
        DataManager.Instance.ServerData.UpdateSubtaskCurrenSuccessValue(
                new UpdateValueBody
                {
    ...
    /// <summary>
    /// 리스너가 이벤트 받아서 어떻게 처리할지 알아서 정의하기
    /// </summary>
