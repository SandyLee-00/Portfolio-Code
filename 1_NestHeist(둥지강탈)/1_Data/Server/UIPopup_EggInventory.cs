...
public class UIPopup_EggInventory_Presenter
{
    ...
    public void SelectEgg(ServerUserEggData egg)
    {
        ...
        GameManager.Instance.HatchingManager.SelectedEgg(egg);
    ...
    private void SetSubItem()
    {
        ...
        List<ServerUserEggData> userInventoryList = DataManager.Instance.ServerData.UserEggDataList;
    ...
    private void SetText()
    {
        ...
        {
            _needGauge = DataManager.Instance.BaseData.EggInfo[_selectedEgg.EggId].NeedGauge;
            needGauge = _needGauge.ToString();
        }
        else
        {
            needGauge = "0";
    ...
    private void SetSprite()
    {
        ...
        if (_selectedEgg != null)
        {
            egg = ResourceManager.Instance.Load<Sprite>($"Icon/EggIcon/{_selectedEgg.EggId}");
        }
        else
        {
    ...
    {
        if (DataManager.Instance.ServerData.UserEggDataList.Count == 0)
        {
            return null;
        }

        return DataManager.Instance.ServerData.UserEggDataList[0];
    }
    ...
    private void OnSmallNutrient()
    {
        ...
        int smallNutrient = DataManager.Instance.ServerData.UserInventoryList.Find(x => x.ItemId == Define.SmallNutrient).Quantity;
    ...
    private void OnMediumNutrient()
    {
        ...
        const int mediumValue = 2;

        int mediumNutrient = DataManager.Instance.ServerData.UserInventoryList.Find(x => x.ItemId == Define.MediumNutrient).Quantity;
    ...
    private void OnLargeNutrient()
    {
        ...
        const int largeValue = 3;

        int largeNutrient = DataManager.Instance.ServerData.UserInventoryList.Find(x => x.ItemId == Define.LargeNutrient).Quantity;
    ...
    /// </summary>
    private async void OnClickStartHatchButton()
    {
        if (_nutrientSum < _needGauge)
        {
            UIPopup_Warning popup = UIManager.Instance.InstanciateUIPopup<UIPopup_Warning>();
        ...
        {
            await StartHatchingProcess();
        }
    }

    private async Task StartHatchingProcess()
    {
        await LoadingManager.Instance.RunWithLoading(async () =>
        {
            await DataManager.Instance.ServerData.StartHatchEgg(new StartHatchingBody
            {
                Id = GameManager.Instance.HatchingManager.IncubatorId,
                ...
                {
                new ItemMaterial { ItemId = Define.SmallNutrient, Quantity = _smallNutrientCount },
                new ItemMaterial { ItemId = Define.MediumNutrient, Quantity = _mediumNutrientCount },
                new ItemMaterial { ItemId = Define.LargeNutrient, Quantity = _largeNutrientCount }
                },
                hoursToHatch = DataManager.Instance.BaseData.EggInfo[_selectedEgg.EggId].HatchTime

            });
        },
        onLoaded: () =>
        {
            GameManager.Instance.HatchingManager.StartHatching(_smallNutrientCount, _mediumNutrientCount, _largeNutrientCount);
        ...
        {
            // 서버 요청 실패 시 롤백
            Logging.LogError($"UIPopup_EggInventory::OnClickStartHatchButton()  실패: {message}");
        });
    }
}